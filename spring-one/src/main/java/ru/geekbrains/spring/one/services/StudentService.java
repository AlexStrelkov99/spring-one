...
public class StudentService {
    ...
    public void decreaseScores(Long id){
        Optional<Student> student = studentRepository.findOneById(id);
        if(student.isPresent() && student.get().getScore()>0){
            studentRepository.decreaseScores(id);
        }
    }
    public void increaseScores(Long id){
        Optional<Student> student = studentRepository.findOneById(id);
        if(student.isPresent() && student.get().getScore()<200){
            studentRepository.increaseScores(id);
        }
    }
}
